...
PatternSpectrumGUI::PatternSpectrumGUI(QWidget *parent)
	: QMainWindow(parent), spectrum(new PatternSpectrum(0, 0.1))
{
	ui.setupUi(this);
...
PatternSpectrumGUI::~PatternSpectrumGUI()
{
	if (spectrum != NULL)
		delete spectrum;
}

void PatternSpectrumGUI::openImageButtonClicked()
{
	QFileDialog* fileDialog = new QFileDialog(this);
	
	fileDialog->setFileMode(QFileDialog::ExistingFile);
	fileDialog->setNameFilter("Изображения (*.png *.jpg *.bmp *.tif);; Контуры в текстовом формате (*.txt)");
...
void PatternSpectrumGUI::checkBoxesChanged(int)
{
	bool flag = false;
...
void PatternSpectrumGUI::radiusChanged(double rad){
	ui.Painter->radius = rad;
	ui.Painter->makeVisualization();
}

void PatternSpectrumGUI::scaleChanged(int sc){
	ui.Painter->scope = sc/100.0;
	ui.Painter->makeVisualization();
}

void PatternSpectrumGUI::updateSkeleton()
{
	if (filepath.size() != 0) {
...
void PatternSpectrumGUI::DrawPlot(){
...
void PatternSpectrumGUI::saveImage()
{
...
void PatternSpectrumGUI::savePolygons()
{
