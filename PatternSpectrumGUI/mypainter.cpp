		...
		if (drawBones) {
			...
			if (makeLacunas) {
				...
				for (auto iLacn : skeleton->Lacunas) {
					...
					for (auto iBone : iLacn->Bones) {
						double xMin = iBone->org->X();
						double yMin = iBone->org->Y();
						double xMax, yMax;
						if (iBone->dest->r() > radius) {
							QPointF Extr = iBone->GetExtremePoint(radius);
							xMax = Extr.x();
				...
				pen = QPen(Qt::red, image.width() / 1000.0, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
				for (auto iBone : skeleton->AllBones) {
				...
				pen = QPen(Qt::blue, image.width() / 1000.0, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
				for (auto iBone : skeleton->AllBones) {
				...
				pen = QPen(Qt::green, image.width() / 1000.0, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
				for (auto iBone : skeleton->AllBones) {
					if (!iBone->Adjacent.empty()) {
						QPointF A = iBone->GetExtremePoint(radius);
